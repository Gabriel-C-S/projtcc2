...
// define o offset base dependendo do caderno
$baseOffset = ($caderno === 2) ? 90 : 0;
$offset = isset($_GET['offset']) ? (int)$_GET['offset'] : $baseOffset;
...
if ($hasLanguages && !$lang) {
    ...
    echo "<br><a href='?year={$year}' class='back-btn'>← Voltar</a>";
...
// cada caderno tem 90 questões → 9 páginas de 10
$totalPages = ceil(90 / $limit);
$currentPage = floor(($offset - $baseOffset) / $limit) + 1;
?>
            ...
            <div class="image-gallery">
                <?php foreach ($question['files'] as $file): ?>
                    <?php
                        ...
                        $fileUrl = str_starts_with($file, 'http') ? $file : "https://api.enem.dev" . $file;
                    ?>
                    <img src="<?php echo htmlspecialchars($fileUrl); ?>" alt="Imagem da questão" />
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
...
<div class="pagination">
    <?php if ($currentPage > 1): ?>
        <a href="?year=<?php echo $year; ?>&offset=<?php echo $offset - $limit; ?>&lang=<?php echo $lang; ?>&caderno=<?php echo $caderno; ?>">&laquo; Anterior</a>
    <?php else: ?>
        <a class="disabled" href="#">Anterior</a>
    <?php endif; ?>

    <?php if ($currentPage < $totalPages): ?>
        <a href="?year=<?php echo $year; ?>&offset=<?php echo $offset + $limit + 1; ?>&lang=<?php echo $lang; ?>&caderno=<?php echo $caderno; ?>">Próximo &raquo;</a>
    <?php else: ?>
...
function finalizarProva(){
    ...
    if(ordenadas.length===0){ resultado+="<li>Você não respondeu nenhuma questão.</li>"; }
        ...
        resultado+=`<div style="max-width:500px;margin:20px auto;"><canvas id="graficoResultado""text-color:black"></canvas></div>`;
        setTimeout(()=>{
 ...
 <br>
    <a class="voltar" href="index.php">←VOLTAR PARA SELEÇÃO DE SIMULADOS</a>
</body>
</html>
